...
Canonical author identity: resolve every Sanjay Prabhu author reference to `https://dolphincentrifuge.com/authors/sanjay-prabhu/#person`. Use `Article.author` when Article schema exists. If there is no Article schema, use `WebPage.author`; do not create Article schema only to carry author data. The canonical profile visibly publishes the source-backed University of Arkansas credential and evidence links. Do not add a class year or maintain a second page-local biography.
