...
if(progress.archiveEncoding!=='gzip-base64'){await query('DELETE FROM source_archive WHERE release=?',[release]);progress={offset:Math.min(progress.offset,kb.docs.length),archiveEncoding:'gzip-base64'};await query('INSERT OR REPLACE INTO knowledge_meta VALUES (?,?)',[progressKey,JSON.stringify(progress)]);}
...
for(const [index,source] of kb.info.sourceFiles.entries()){
 const text=gzipSync(Buffer.from(kb.sourceContents[index],'utf8')).toString('base64');
...
for(const source of kb.info.sourceFiles){const chunks=[];for(let offset=0;;offset+=20){const rows=(await query('SELECT content FROM source_archive WHERE release=? AND name=? ORDER BY part LIMIT 20 OFFSET ?',[release,source.name,offset]))[0].results;for(const row of rows)chunks.push(row.content);if(rows.length<20)break;}if(createHash('sha256').update(gunzipSync(Buffer.from(chunks.join(''),'base64'))).digest('hex')!==source.sha256)throw new Error('Cloud source hash mismatch: '+source.name);console.log('Verified '+source.name);}
const metadata={release,info:{...kb.info,hosting:'cloud',passages:count,sourceArchiveEncoding:'gzip-base64'},writing:{guide:kb.writingGuide,examples:kb.wording}};
