...
for(const [index,source] of kb.info.sourceFiles.entries()){const text=kb.sourceContents[index];for(let at=0,part=0;at<text.length;at+=24000,part++)await add(`INSERT INTO source_archive VALUES (${quote(release)},${quote(source.name)},${part},${quote(text.slice(at,at+24000))})`);await flush();console.log('Archived '+source.name);}
